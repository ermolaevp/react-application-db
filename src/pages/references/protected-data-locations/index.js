...
const ProtectedDataLocations = ({
  ...
  return (
    <MainLayout title={title}>
      {currentUser.isModerator && <AddButton onClick={visit('/references/protected-data-locations/add')} />}
      <Form model="protectedDataLocationFilterForm" onSubmit={submit}>
        <Table>
          ...
          <TableBody displayRowCheckbox={false} showRowHover>
            {protectedDataLocations.map(entity =>
              <TableRow
                key={entity.id}
                onTouchTap={visit(`/references/protected-data-locations/show/${entity.id}`)}
              >
                <TableRowColumn>
                  {entity['name']}
                </TableRowColumn>
                <TableRowColumn style={{ textAlign: 'right' }} onTouchTap={e => e.stopPropagation()}>
                  {currentUser.roles.includes('moderator') && <IconButton onClick={visit(`/references/protected-data-locations/edit/${entity.id}`)}>
                    <EditIcon />
                  </IconButton>}
                </TableRowColumn>
              </TableRow>
...
ProtectedDataLocations.propTypes = {
  currentUser: PropTypes.object.isRequired,
  protectedDataLocations: PropTypes.array.isRequired,
...
export default smartify(ProtectedDataLocations);
