...
const selector = createStructuredSelector({
  ...
  protectedDataLocations: state => state.protectedDataLocations.results,
});

const mapDispatchToProps = (dispatch, props) => ({
  load: (search = {}) => {
    const order = props.getOrderObject();
    dispatch(api.actions.protectedDataLocations.get({ ...order, ...search }));
  },
  submit: (model) => {
    const res = {};
    ...
    props.history.push('/references/protected-data-locations?' + queryString.stringify(res));
  },
});
