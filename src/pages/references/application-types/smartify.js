...
const selector = createStructuredSelector({
  ...
  osTypes: state => state.osTypes.results,
  applicationTypes: state => state.applicationTypes.results,
});
...
const mapDispatchToProps = (dispatch, props) => ({
  load: (search = {}) => {
    ...
    dispatch(api.actions.applicationTypes.get({ select: '*,os_types(id,name)', ...order, ...search }));
  },
  submit: (model) => {
    const res = {};
    ...
    props.history.push('/references/application-types?' + queryString.stringify(res));
  },
});
