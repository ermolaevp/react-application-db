...
const ProtectedDataTypes = ({
  ...
  return (
    <MainLayout title={title}>
      {currentUser.isModerator && <AddButton onClick={visit('/references/protected-data-types/add')} />}
      <Form model="protectedDataTypeFilterForm" onSubmit={submit}>
        <Table>
          ...
          <TableBody displayRowCheckbox={false} showRowHover>
            {protectedDataTypes.map(entity =>
              <TableRow
                key={entity.id}
                onTouchTap={visit(`/references/protected-data-types/show/${entity.id}`)}
              >
                <TableRowColumn>
                  {entity['name']}
                </TableRowColumn>
                <TableRowColumn style={{ textAlign: 'right' }} onTouchTap={e => e.stopPropagation()}>
                  {currentUser.roles.includes('moderator') && <IconButton onClick={visit(`/references/protected-data-types/edit/${entity.id}`)}>
                    <EditIcon />
                  </IconButton>}
                </TableRowColumn>
              </TableRow>
...
ProtectedDataTypes.propTypes = {
  currentUser: PropTypes.object.isRequired,
  protectedDataTypes: PropTypes.array.isRequired,
...
export default smartify(ProtectedDataTypes);
